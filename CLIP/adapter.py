...
#             [ClipAdapter(1024, bottleneck=768) for _ in range(len(features))]
#         )
...
#     def forward(self, x):
#         # Ensure x is batched: (batch_size, channels, height, width)
#         x = self.image_encoder.conv1(x)  # Output shape: (batch_size, channels, height, width)
#         x = x.reshape(x.shape[0], x.shape[1], -1)  # Flatten spatial dimensions
#         x = x.permute(0, 2, 1)  # Shape: (batch_size, num_patches, channels)

#         # Add class embedding and positional embedding
#         batch_size = x.shape[0]
#         x = torch.cat(
#             [self.image_encoder.class_embedding.to(x.dtype)
#              + torch.zeros(batch_size, 1, x.shape[-1], dtype=x.dtype, device=x.device),
#              x], dim=1
#         )
#         x = x + self.image_encoder.positional_embedding.to(x.dtype)
#         x = self.image_encoder.patch_dropout(x)
#         x = self.image_encoder.ln_pre(x)
#         x = x.permute(1, 0, 2)  # Transformer expects (sequence_length, batch_size, channels)

#         attn_out = []
#         seg_patch_tokens = []
#         det_patch_tokens = []

#         for i in range(24):  # Process 24 transformer layers
#             x, attn_map = self.image_encoder.transformer.resblocks[i](x, attn_mask=None)
#             if i + 1 in self.features:
#                 if isinstance(x, list):
#                     x = torch.stack(x)

#                 # Apply adapters for segmentation and detection
#                 seg_adapt_med, seg_adapt_out = self.seg_adapters[self.features.index(i + 1)](x)
#                 det_adapt_med, det_adapt_out = self.det_adapters[self.features.index(i + 1)](x)
...
#         x = x.permute(1, 0, 2)  # Revert to (batch_size, sequence_length, channels)
#         seg_patch_tokens = [seg_patch_tokens[t].permute(1, 0, 2) for t in range(len(seg_patch_tokens))]
#         det_patch_tokens = [det_patch_tokens[t].permute(1, 0, 2) for t in range(len(det_patch_tokens))]
